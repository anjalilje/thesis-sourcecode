...
parser.add_argument('-o', '--out_size', nargs=1, type=int, default=224, help='Out size of images')
parser.add_argument('-d', '--dscl', nargs=1, type=int, default=150, help='Downscale image for STN')
...
for i, batch_test_idxs in enumerate(batch(all_test_indices,64)):
    ...
    batch_test_xs_low = (batch_test_xs.copy()).astype(np.uint8)
    batch_test_xs_low = (np.array([resize(x, (down_scale, down_scale)) for x in batch_test_xs_low])).astype(np.float32)

    batch_test_xs = batch_test_xs/255.0
    labels_test, masks_test = generate_masks_and_labels(batch_test_ys)

    feed = {x: batch_test_xs, x_low: batch_test_xs_low, m: masks_test, y: labels_test, dropout_rate: 0.0}
